function [nodes, domainCounts, order, rnodes] = dag2randBNprior(graph, type, priors, varargin)
...
if isequal(type, 'discrete')
    [minNumStates, maxNumStates,   headers, alpha,confounder] = process_options(varargin, 'minNumStates', 2*ones(1, numNodes), 'maxNumStates', 5*ones(1,numNodes),  'headers', ...
        arrayfun(@(x)sprintf('x%i',x),1:numNodes, 'uniformOutput',false), 'alpha',ones(1, numNodes), 'confounder', false);
...
if isequal(type, 'discrete')
    ...
    for i = order
        ...
        if isempty(node.parents)
           ...
           [theta, theta2] = dirichletsample(priors{i}.priors,1);
            node.cpt = theta;
            rnode.cpt = theta2;
        else
            [theta, theta2] =dirichletsample(priors{i}.priors, domainCounts(node.parents));
            node.cpt = theta;
            rnode.cpt = theta2;
        end
...
for j=1:N
    for i=1:k
        theta(j,i) = gamrnd(alpha(j,i)*100, scale, 1, 1);
    end
end
S = sum(theta,2);
