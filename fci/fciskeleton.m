...
while n<maxCondSetSize 
    ...
    if pdSep==1
        [pag, ~, ~, ~] = orientunshieldedcolliders(pag, sepSet, verbose);
        [Ys, Xs] = find(triu(pag));
        for iEdge=1:length(Xs)
            X =Xs(iEdge); Y = Ys(iEdge);
            fprintf('Checking %d-%d, p-value %.3f\n', X, Y, edgepvals(iEdge));
            pdSepSet =  findpdsepset(pag, curX, curY, verbose);
            if verbose 
                fprintf('Set [%s] is a possibly d separating set for %d-%d\n', num2str(pdSepSet), curX, curY);
            end
            n=3;found=false;
            while ~found && n<maxCondSetSize
                ...
                for iPDS =1:size(pdSepSets, 1)
                    ...
                    if all(pag(X, condset)) || all(pag(Y, condset)) 
                        continue;
                    else
                        fprintf('\t with set %s, strength %.3f\n', num2str(condset), strengthCondsets(uniqueOrd(iCS), :));
                        [p, ~, exitflag] = feval(test,X, Y, condSet, dataset);
                        if p>alpha 
                            found=true;
                            pag(X, Y) = 0; pag(Y, X) = 0;
