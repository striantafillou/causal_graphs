function allpaths = onapathbetweenxandy(mcg,nodes, Y)
...
allpaths = {};
allpaths = zeros(nVars^2, nVars);
pathCount = 0;

for l =1:length(nbrs)        
    null = testMConnections2(mcg,nodes,nbrs(l),1,[],[nodes nbrs(l)],Y,nodes);
end

    function null=testMConnections2(pag,U,V,i,visitedTriples,curPath,Y, X)
        null = [];
            ...
            if W~=U && (isempty(visitedTriples)||~ismember([U V W],visitedTriples,'rows')) && ~ismember(W,curPath) && W~= X
                 if (~iscollider(mcg, U, V,W) && ~inConditionSet(V)) || iscollider(mcg, U, V, X) && descendents(V);
                    if W~=Y
                     ...
                     testMConnections2(pag,V,W,i+1,visitedTriples,[curPath W],Y, X);
                    elseif W == Y
                        pathCount = pathCount + 1;
                        allpaths(pathCount, 1:length(curPath)) = curPath;%deal(1:length(curPath));%{pathCount} = [curPath Y];
