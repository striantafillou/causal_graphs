function [bestMag, bestScore, iter, gs] = greedySearchMagTABUInit(covMat, nSamples, tol, initMag)
    nVars=size(covMat,1);
    ...
    mag =initMag; bestMag = mag;
    mag_bidir = mag==2&mag'==2;
    [nComps, sizes, comps, inComponent]= concomp(mag_bidir);
    nsf = -nSamples/2;
    ...
    noChange=0;
    % Initialize TABU
    isTabu = false(nPairs, 4);
    tabu_cnt = 0;
    tabuOrd = zeros(nPairs,4);
    ...
    while noChange<50
        ...
        for iPair =1:nPairs
            ...
            % component (from);
           % fprintf(' \t 1 \t')
            if allowed(iPair, 1) && ~isTabu(iPair, 1)
            ...
            % add edge to->from
            %fprintf(' \t 2 \t')
            if allowed(iPair, 2) && ~isTabu(iPair, 2)
            ...
            % add edge from<->to        
            %fprintf(' \t 3 \t')
            if allowed(iPair, 3) && ~isTabu(iPair, 3)
                % update components
              [stepScores(iPair, 3), scoreContribs{iPair, 3}] = ...
               ...
               %else fprintf('\t %d<->%d not allowed\n', from, to);
            %fprintf(' \t 4 \n')
            if allowed(iPair, 4) && ~isTabu(iPair, 4)
        ...
        % update tabu list
        tabu_cnt = tabu_cnt+1;
        tabuListInd = mod(tabu_cnt, 100)+1;
        tabuRem = tabuOrd==tabuListInd;
        %remove 100-th element
        tabuOrd(tabuRem) = 0;
        isTabu(tabuRem) = 0;
        % push new element     
        tabuOrd(whichPair, actions(whichPair))= tabu_cnt;
        isTabu(whichPair, actions(whichPair))= true;
