...
//        return simulateDataAcyclic(sampleSize, latentDataSaved);
    }
    ...
    public DataSet simulateData(int sampleSize, boolean latentDataSaved,
        ...
        return simulateDataCyclic(sampleSize, random, latentDataSaved);
    }
    ...
    public DataSet simulateDataAcyclic(int sampleSize,
        ...
        Distribution gaussian = new Normal(0, 1);
        Distribution gaussianPower = new GaussianPower(1, 4);
        Distribution studentt = new StudentT(2);
        Distribution lognormal = new LogNormal(1);
        Distribution mixture = new MixtureOfGaussians(.5, -2, .5, 2, .5); // Doesn't work
        Distribution uniform = new Uniform(-100, 100); // Doesn't work.
        Distribution chisquare = new ChiSquare(10);
        Distribution indicator = new Indicator(.7); // Doesn't work.
        Distribution beta = new Beta(5, 3);
        Distribution discrete = new Discrete(.1, .7, .2);

        Distribution[] distributions = new Distribution[variableNodes.size()];

        for (int i = 0; i < variableNodes.size(); i++) {
            boolean b = RandomUtil.getInstance().nextDouble() > 1;
            distributions[i] = b ? gaussian : mixture;
        ...
        for (int row = 0; row < sampleSize; row++) {
            for (int i = 0; i < tierOrdering.size(); i++) {
                ...
                double error = RandomUtil.getInstance().nextNormal(0, 1);
                double value = error * errCovar().get(col, col);

//                Node variable = fullDataSet.getVariable(col);
//                double value = Double.NaN;
//
//                if (variable.getNodeType() == NodeType.MEASURED) {
                    value = distributions[i].nextRandom();
//                }
//                else if (variable.getNodeType() == NodeType.LATENT) {
//                    value = gaussianPower.drawSample();
//                }
//                else {
//                    //
//                }

//                double value = lognormal.drawSample();
...
//    }
    ...
    public DataSet simulateDataCyclic(int sampleSize, boolean latentDataSaved) {
...
//    }
     ...
     */
    private DataSet simulateDataCyclic(int sampleSize,
