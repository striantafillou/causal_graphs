...
/**
 * Implements the DCI (Distributed Causal Inference) algorithm for learning
 * causal structure over a set of variable from multiple datasets that each
 * may only measure proper overlapping subsets subsets of that sets, or
 * datasets with some variables in common and others not. The algorithm
 * currently takes as input a set of PAGs (presumably learned using a local
 * learning algorithm such as FCI) and returns a complete set of PAGs over
 * every variable included a dataset that are consistent with all of the
 * PAGs (same d-separations and d-connections). This version resolves
 * inconsistencies using a specified method.
 *
...
public class DciR {
    ...
    /**
     * Graph change status and sets of graphs resulting from initial and
    ...
    /**
     * Keeps up with current graph while searching for possible unshielded
     * sets and
     */
    ...
    /**
     * The method used to resolve inconsistencies
     */
    ...
    public List<Graph> search() {
        ...
        // resolve inconsitencies
        SepsetMapDci resolvedSepset = ResolveSepsets.ResolveSepsets(sepsetMaps,
                independenceTests, method, new SepsetMapDci(), new SepsetMapDci());
        sepsetMaps.clear();
        sepsetMaps.add(resolvedSepset);
        // now remove edges
    ...
    private void orientColliders(Graph graph) {
        for (int k=0; k<marginalVars.size(); k++) {
            ...
            SepsetMapDci sepset = sepsetMaps.get(0);
    ...
    /**
     * Implements the double-triangle orientation rule for the initial graph
     * with only definite colliders from each dataset oriented, which states
     * that if D*-oB, A*->B<-*C and A*-*D*-*C is a noncollider, which A, B and C
    ...
    /**
     * Finds the discriminating paths relative only to variables measured
     ...
     * The body of a DDP consists of colliders that are parents of c. This
     * only considers discriminating paths that are completely jointly measured.
     */
    ...
    /**
     * Finds a minimal spanning set of treks for the nodes in a particular
     * "marginal" dataset
     */
    private Map<List<Node>, Set<Node>> minimalSpanningTreks(Graph graph, Set<Node> marginalNodes) {
        int size = marginalNodes.size();
        Map<Integer, Map<List<Node>, Set<Node>>> allPaths = new HashMap<Integer, Map<List<Node>, Set<Node>>>();
        for (int k = 2; k <= size; k++) {
            allPaths.put(k, new HashMap<List<Node>, Set<Node>>());
        }
        for (NodePair nodePair : allNodePairs(new ArrayList<Node>(marginalNodes))) {
            if (graph.isAdjacentTo(nodePair.getFirst(), nodePair.getSecond())) {
                Set<Node> otherNodes = new HashSet<Node>(marginalNodes);
            ...
            for (List<Node> trek : GraphUtils.treks(graph, nodePair.getFirst(), nodePair.getSecond())) {
                ...
                for (Node node : trek) {
                    if (!marginalNodes.contains(node)) {
                        inMarginal = false;
                        break;
                    }
                }
...
//                allPaths.get(trek.size()).put(trek, otherNodes);
    ...
    private void getTriplesDefiniteColliders(Graph graph) {
        ...
        for (Triple triple : allTriples) {
            if (graph.isDefiniteCollider(triple.getX(), triple.getY(), triple.getZ())) {
    ...
    /**
     * Finds all edge sequences that ensure a particular trek. Returns the edge
     * sequence with necessary colliders along the sequence mapped to ancestral
     * paths
     */
       ...
       */             Set<Edge> edges = new HashSet<Edge>();
        ...
        for (Edge edge : graph.getEdges(node1)) {
            ...
            if (path.size() > 1) {
                ...
                if (!next.equals(node0)&&graph.isDefiniteCollider(node0, node1, next) && !conditioning.contains(node1)) {
                    continue;
                }
            }
    ...
    /**
     * For each necessary trek in a marginal graph, finds the treks in the current graph
     * that traverse each node in the trek using only intermediary nodes not present in the
     * marginal graph
     */
    ...
    private void allColliderCombinations(Graph graph, Set<Edge> removedEdges, Set<Triple> newColliders, int skeletonsLeft) {
        ...
        for (Triple triple : allTriples) {
            if (!graph.isAdjacentTo(triple.getX(), triple.getZ()) && !definiteNoncolliders.contains(triple)
                    && !graph.isDefiniteCollider(triple.getX(), triple.getY(), triple.getZ()) &&
                    ...
                    for (Triple triple : newSet) {
                        if (path.contains(currentGraph.getEdge(triple.getX(), triple.getY())) && path.contains(currentGraph.getEdge(triple.getY(), triple.getZ()))
                            ...
                            for (Triple collider : necessaryTrek.get(path).keySet()) {
                                if (!currentGraph.isDefiniteCollider(collider.getX(), collider.getY(), collider.getZ()) &&
    ...
    /**
     * Checks to make sure a graph entails d-separations relations from
     * the sepsetMap
     */
