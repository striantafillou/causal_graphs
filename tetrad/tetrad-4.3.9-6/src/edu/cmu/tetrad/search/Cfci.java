...
/**
 * Implements the Conservative FCI algorithm. This is an (as yet undocumented)
 * algorithm in which FCI is modified to use conservative orientation rules
 * as motivated by the CPC algorithm.
 *
                ...
                if (cb == Endpoint.CIRCLE || cb == Endpoint.TAIL) {
                    if (bc == Endpoint.CIRCLE) {
                        ...
                        if (getNoncolliderTriples().contains(new Triple(a, b, c))) {
                            ...
                            TetradLogger.getInstance().edgeOriented(SearchLogUtils.edgeOrientedMsg("Away from collider",
