...
import edu.cmu.tetrad.search.indtest.IndTestSepset;
 ...
 * PAGs (same d-separations and d-connections)
 *
...
public class Dci {
    ...
    public Dci(List<IndependenceTest> tests, String method) {
        Set<Node> variables = new HashSet<Node>();
        for (IndependenceTest test : tests) {
            if (test == null) {
                throw new NullPointerException();
     ...
     */
    public void setPoolingMethod(String method) {
        this.method = method;
    }
...
//                  allPaths.get(trek.size()).put(trek, otherNodes);
    ...
    /**
     * Resolves possibly conflicting independence/dependence statements that
     * result from a set of known independence statements
     */
...
//            condSet.remove(remove);
    ...
    private void resolveResultingIndependenciesC() {
        ...
        SepsetMap fciSepset = fci.getSepset();
        for (int k=0; k<marginalVars.size(); k++) {
