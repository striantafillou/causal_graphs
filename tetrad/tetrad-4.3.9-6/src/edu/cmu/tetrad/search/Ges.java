...
public final class Ges implements GraphSearch {
    /**
...
//
//                        if (trueGraph != null) {
//                            Node x2 = trueGraph.getNode(_x.getName());
//                            Node y2 = trueGraph.getNode(_y.getName());
//                            trueEdge = trueGraph.getEdge(x2, y2);
//                        }
//
//                        String label = trueGraph != null && trueEdge != null ? "*" : "";
//                        TetradLogger.getInstance().details(graph.getNumEdges() + ". Attempt adding  " +
//                                 _x + "-->" + _y + tSubset + " (" + nf.format(evalScore) + ") " + label);

                        if (!(evalScore > bestScore && evalScore > score)) {
                            continue;
                        }

//                        double minJump = sampleSize * Math.log(1.9) - Math.log(sampleSize);
//
...
//        return -n * Math.log(Math.sqrt(variance)) - n * Math.log(2 * Math.PI) - 0.5 * n - getPenaltyDiscount() * d * Math.log(n);
//        return -n * Math.log(Math.sqrt(variance)) - n * Math.log(2 * Math.PI) - 0.5 * n - getPenaltyDiscount() * d * Math.log(n);
