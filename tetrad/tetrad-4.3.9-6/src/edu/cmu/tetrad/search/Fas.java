 ...
 * edges in the graph and for each depth d = 0, 1, 2, ..., d1, where d1 is
 * either the maximum depth or else the first such depth at which no edges can
 * be removed. The interpretation of this adjacency search is different for
 * different algorithms, depending on the assumptions of the algorithm. A
 * mapping from {x, y} to S({x, y}) is returned for edges x *-* y that have been
 * removed.
 *
...
//               .MAX_VALUE) ? "Unlimited" : Integer.toString(depth)));
        ...
        for (int d = 0; d <= _depth; d++) {
            boolean more = searchAtDepth(graph, test, new Knowledge(), sepset, d);
