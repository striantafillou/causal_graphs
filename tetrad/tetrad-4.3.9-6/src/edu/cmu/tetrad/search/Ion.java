...
/**
 * Implements the ION (Integration of Overlapping Networks) algorithm for
 * distributed causal inference. The algorithm takes as input a set of PAGs
 * (presumably learned using a local learning algorithm) over variable sets
 * that may have some variables in common and others not in common. The algorithm
 * returns a complete set of PAGs over every variable form an input PAG that
 * are consistent (same d-separations and d-connections) with every input PAG.
 *
...
public class Ion {
    ...
    public Ion(List<Graph> pags) {
        ...
        for (Graph pag : input) {
            ...
            for (Triple triple : getAllTriples(pag)) {
                if (pag.isDefiniteNoncollider(triple.getX(), triple.getY(), triple.getZ())) {
    ...
    public List<Graph> search() {
        ...
        TetradLogger.getInstance().info("Transfering local information.");
        ...
        for (NodePair pair : nonIntersection()) {
            graph.addEdge(new Edge(pair.getFirst(), pair.getSecond(), Endpoint.CIRCLE, Endpoint.CIRCLE));
        }
        TetradLogger.getInstance().info("Steps 1-2: " + (System.currentTimeMillis() - steps) / 1000. + "s");
...
//            doFinalOrientation(graph);
        ...
        for (int l = pl; l < numNodes; l++) {
            if (pathLengthSearch) {
                TetradLogger.getInstance().info("Braching over path lengths: " + l + " of " + (numNodes - 1));
            }
            ...
            for (IonIndependenceFacts fact : separations) {
                if (adjacencySearch) {
                    TetradLogger.getInstance().info("Braching over path nonadjacencies: " + currentSep + " of " + numAdjacencies);
                }
                seps--;
                    ...
                    for (Set<GraphChange> changes : findChanges(paths)) {
                        ...
                        for (GraphChange gc : changes) {
                            ...
                            for (Triple collider : gc.getNoncolliders()) {
                                if (pag.isDefiniteCollider(collider.getX(), collider.getY(), collider.getZ())) {
                                    okay = false;
                                    break;
                                }
                            }
...
//                        doFinalOrientation(changed);
        ...
        TetradLogger.getInstance().info("Step 3: " + (System.currentTimeMillis() - steps) / 1000. + "s");
        ...
        TetradLogger.getInstance().info("Step 4: " + (System.currentTimeMillis() - steps) / 1000. + "s");
        ...
        for (Graph pag : outputPags) {
            ...
            for (Graph outputPag : finalResult) {
                if (!predictsFalseIndependence(associations, outputPag)) {
                    Set<Triple> underlineTriples = new HashSet<Triple>(outputPag.getUnderLineTriples());
        ...
        TetradLogger.getInstance().info("Step 5: " + (System.currentTimeMillis() - steps) / 1000. + "s");
    ...
    private Set<NodePair> nonadjacencies() {
        ...
        for (Graph graph : input) {
            for (NodePair pair : allNodePairs(graph.getNodes())) {
                if (!graph.isAdjacentTo(pair.getFirst(), pair.getSecond())) {
    ...
    private void transferLocal(Graph graph) {
        Set<NodePair> nonadjacencies = nonadjacencies();
        for (Graph pag : input) {
            for (Edge edge : pag.getEdges()) {
                if (nonadjacencies.contains(new NodePair(edge.getNode1(), edge.getNode2()))) {
                    continue;
                }
            ...
            for (Triple triple : pag.getUnderLineTriples()) {
                ...
                definiteNoncolliders.add(triple);
            }
        }
    }
    ...
    private List<NodePair> nonIntersection() {
        List<Set<Node>> varsets = new ArrayList<Set<Node>>();
        for (Graph graph : input) {
        ...
        for (int i = 0; i < variables.size() - 1; i++) {
            ...
            for (int j = i + 1; j < variables.size(); j++) {
                ...
                Node node2 = variables.get(j);
                for (Set<Node> varset : varsets) {
                    if (varset.containsAll(Arrays.asList(node1, node2))) {
                        intersection = true;
                        break;
                    }
                }
    ...
    private List<Set<IonIndependenceFacts>> findSepAndAssoc() {
        ...
        List<NodePair> allNodes = allNodePairs(this.variables);
    ...
    private Set<Triple> getPossibleTriples(Graph pag) {
        ...
        for (Triple triple : getAllTriples(pag)) {
            if (pag.isAdjacentTo(triple.getX(), triple.getY()) && pag.isAdjacentTo(triple.getY(), triple.getZ())
                    ...
                    !pag.isDefiniteCollider(triple.getX(), triple.getY(), triple.getZ())) {
