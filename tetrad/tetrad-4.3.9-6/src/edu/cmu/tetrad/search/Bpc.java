...
public final class Bpc {
    ...
    public Bpc(CovarianceMatrix covarianceMatrix, double sig,
    ...
    public Bpc(CorrelationMatrix correlationMatrix, double sig,
               int sigTestType, int purifyTestType) {
    ...
    public Bpc(DataSet dataSet, double sig,
               int sigTestType, int purifyTestType) {
...
//            initAlgorithm(sig, sigTestType, purifyTestType);
    ...
    private void initAlgorithm(double alpha, int sigTestType, int purifyTestType) {
        ...
        if (sigTestType == TEST_DISCRETE) {
            ...
            numVariables = getCorrelationMatrix().getSize();
            independenceTest = new IndTestCramerT(getCorrelationMatrix(), alpha);
            int type = -1;
            switch (sigTestType) {
            ...
            if (sigTestType == TEST_TETRAD_BOLLEN || purifyTestType == PURIFY_TEST_TETRAD_BOLLEN) {
                ...
                tetradTest = new ContinuousTetradTest(getCorrelationMatrix(), type,
                        alpha);
            }
        }
    ...
    private void shuffleVariables() {
        ...
        if (getCorrelationMatrix() != null) {
            correlationMatrix = getCorrelationMatrix().getSubCorrMatrix(indices);
        }
        if (dataSet != null) {
    ...
    public static String[] getTestDescriptions() {
        return new String[]{
                "Wishart's Tetrad Test",
                "Bollen's Tetrad Test",
//                "Gaussian Factors Test"
        };
...
//            estimator.estimate();
            ...
            SemEstimator estimator = new SemEstimator(getCorrelationMatrix(),
                    semPm, optimizer);
            estimator.estimate();
...
//            return null;
    ...
    private boolean uncorrelatedPopulation(int v1, int v2) {
        return Math.abs(getCorrelationMatrix().getValue(v1, v2)) < 0.0001;
    }

    private boolean vanishingPartialCorrPopulation(int x, int y, int z) {
        double pc = getCorrelationMatrix().getValue(x, y)
                - getCorrelationMatrix().getValue(x, z) *
                getCorrelationMatrix().getValue(y, z);
        pc /= Math.sqrt(1 - getCorrelationMatrix().getValue(x, z) *
                getCorrelationMatrix().getValue(x, z)) *
                Math.sqrt(1 - getCorrelationMatrix().getValue(y, z) *
                        getCorrelationMatrix().getValue(y, z));
        double r = 0.5 * Math.sqrt(getCorrelationMatrix().getSampleSize() - 4) *
                Math.log((1. + pc) / (1. - pc));
    ...
    private boolean uncorrelated(int v1, int v2) {
        ...
        if (getCorrelationMatrix() != null) {
            ...
            if (getIndependenceTest().isIndependent(
                    getCorrelationMatrix().getVariables().get(v1),
                    getCorrelationMatrix().getVariables().get(v2),
                    new ArrayList<Node>())) {
                System.out.println(getCorrelationMatrix().getVariables()
                        .get(v1) + " " +
                        getCorrelationMatrix().getVariables().get(v2) +
                        " == " + getCorrelationMatrix().getValue(v1, v2));
            }
    ...
    private boolean vanishingPartialCorr(int x, int y, int z) {
        ...
        if (getCorrelationMatrix() != null) {
            ...
            conditional.add(new ContinuousVariable(
                    getCorrelationMatrix().getVariableName(z)));

            if (!getIndependenceTest().isIndependent(
                    getCorrelationMatrix().getVariables().get(x),
                    getCorrelationMatrix().getVariables().get(y),
                    conditional)) {
                return getIndependenceTest().isIndependent(
                        getCorrelationMatrix().getVariables().get(x),
                        getCorrelationMatrix().getVariables().get(y),
            ...
            System.out.println(new StringBuilder().append(getCorrelationMatrix().getVariables()
                    .get(x)).append(" ").append(getCorrelationMatrix().getVariables().get(y)).append(" = ").append(getCorrelationMatrix().getValue(x, y)).append("(").append(getCorrelationMatrix().getVariableName(
                    z)).append(")").toString());
...
//        }
    ...
    private List purify(List actualClusterings, List clusterIds,
                        List forbiddenList) {
        printlnMessage();
        print("** PURIFY: total number of clustering candidates = " +
