...
/**
 * Implements a modified version of the the "fast adjacency search" for use in
 * the Distributed Causal Inference (DCI) algorithm. This version accepts
 * an independence test for a particular dataset and a supergraph containing
 * varialbes from each dataset. At a given stage, an edge X*-*Y is removed
 * from the graph if X _||_ Y | S, where S is a subset of size d either of adj(X)
 * or of adj(Y), where d is the depth of the search. This procedure  is
 * performed for each pair of adjacent edges in the graph that are jointly
 * measured with S in the dataset and for d = 0, 1, 2, ..., d1, where d1 is
 * either the maximum depth or else the first such depth at which no edges can
 * be removed. A mapping from {x, y} to S({x, y}) is returned for edges x *-* y
...
public class FasDci {
    ...
    /**
     * The search graph over every variable included in a dataset. It is assumed
     * going in that all of the true adjacencies of x are in this graph for every
     * node x. It is hoped (i.e. true in the large sample limit) that true
    ...
    /**
     * The independence tests for each dataset. This should be appropriate to
     * the data.
     */
     ...
     * independence test. If the depth is -1, it will be taken to be the maximum
     * value, which is 1000. Otherwise, it should be set to a non-negative
     * integer.
     */
    ...
    /**
     * If resolving independencies, the sets of variables in each "Marginal"
     * dataset
     */
    private List<Set<Node>> marginalVars;

    /**
     * If resolving independenceis, the set of independence tests for other
     * datasets
     */
...
//    private List<Double> pValues = new ArrayList<Double>();
    ...
    /**
     * Constructs a new FastAdjacencySearch for DCI with independence test
     * pooling to resolve inconsistencies.
     */
    public FasDci(Graph graph, IndependenceTest independenceTest,
                  String method, List<Set<Node>> marginalVars,
