...
public final class Images implements GraphSearch {
    /**
    ...
    public Graph search() {
        ...
        // Check for missing values.
        for (int i = 0; i < dataSets.size(); i++) {
            if (dataSets.get(i) != null && DataUtils.containsMissingValue(dataSets.get(i))) {
                throw new IllegalArgumentException(
                        "Please remove or impute missing values first.");
            }
        }
...
//    }
    ...
    private double localSemScore(int i, int[] parents) {
        ...
        for (int dataIndex = 0; dataIndex < dataSets().size(); dataIndex++) {
            sum += localSemScoreOneDataSet(dataIndex, i, parents, dataIndex);
        }
...
//        return sum / (double) dataSets().size();
    ...
    private double localSemScoreOneDataSet(int nodeIndex, int i, int[] parents, int dataIndex) {
        // Calculate the unexplained variance of i given z1,...,zn
        // considered as a naive Bayes model.
        double variance = getCorrMatrices().get(nodeIndex).get(i, i);
//        int n = getCorrMatrix().columns();
        ...
        if (parents.length > 0) {
            ...
            DoubleMatrix2D Czz =
                    getCorrMatrices().get(nodeIndex).viewSelection(parents, parents);
            ...
            DoubleMatrix1D Cyz = getCorrMatrices().get(nodeIndex).viewColumn(i);
...
//        return -(n / 2.) - (n / 2.) * Math.log(variance) -
//                penalty * (d / 2.) * Math.log(n);

        int sum = 0;
        int m = dataSets.size();

        for (int r = 0; r < m; r++) {
            sum += dataSets.get(r).getNumRows();
        }

//        return -n * Math.log(variance) - penalty * d * Math.log(dataSets().size() * n);

        // Notice when it sums it up for all data sets it will be m * d parameters.
        return -n * Math.log(variance) - penalty * d * Math.log(n);
//        return -n * Math.log(variance) - penalty * d * Math.log(sum);
    }
...
//        return dataSet().getNumRows();
    ...
    private List<DoubleMatrix2D> getCorrMatrices() {
        return variances;
    }
