...
public final class EdgeListGraph implements Graph {
    ...
    /**
     * A list of ambiguous node pairs.
     */
    ...
    public EdgeListGraph(Graph graph) throws IllegalArgumentException {
        ...
        this.underLineTriples = graph.getUnderLineTriples();
        this.dottedUnderLineTriples = graph.getDottedUnderLineTriples();
        this.ambiguousPairs = new HashSet<Pair>(graph.getAmbiguousPairs());
    }
...
//        GraphUtils.arrangeInCircle(this, 200, 200, 150);
    ...
    public boolean isDefiniteNoncollider(Node node1, Node node2, Node node3) {
        if (isDirectedFromTo(node2, node1) || isDirectedFromTo(node2, node3)) {
            ...
            return (getEndpoint(node1, node2) == Endpoint.CIRCLE &&
                    getEndpoint(node2, node3) == Endpoint.CIRCLE);
        } else {
            return false;
        }
    }
...
*/
    ...
    public boolean isAmbiguous(Node x, Node y) {
        return ambiguousPairs.contains(new Pair(x, y));
    }
    ...
    public boolean isUnderlineTriple(Node x, Node y, Node z) {
        Triple _triple = new Triple(x, y, z);
        return underLineTriples.contains(_triple);
    }
    ...
    public void setUnderLineTriples(Set<Triple> triples) {
        ...
        this.underLineTriples = new HashSet<Triple>(triples);
    }
    ...
    public void setDottedUnderLineTriples(Set<Triple> triples) {
        ...
        this.dottedUnderLineTriples = new HashSet<Triple>(triples);
    }
...
//        buf.append("\nEdges: ");
        ...
        Edges.sortEdges(this.edges);

        for (int i = 0; i < this.edges.size(); i++) {
            Edge edge = this.edges.get(i);
            boolean isAmbiguous = isAmbiguous(edge.getNode1(), edge.getNode2());
            buf.append("\n").append(i + 1).append(". ").append(edge)
                    .append(" ").append(isAmbiguous ? "(Ambiguous)" : "");
        }
...
//        Set<Triple> ambiguousTriples = getAmbiguousTriples();
        ...
        if (!ambiguousPairs.isEmpty()) {
            buf.append("Ambiguous pairs: \n");

            for (Pair triple : ambiguousPairs) {
                buf.append("\t").append(triple).append("\n");
            }
        }
...
//            Edge edge1 = getEdge(triple.getX(), triple.getY());
//            Edge edge2 = getEdge(triple.getY(), triple.getZ());
//
//            if (edge1 == null) {
...
//        }
    ...
    public Set<Pair> getAmbiguousPairs() {
        return new HashSet<Pair>(ambiguousPairs);
    }
