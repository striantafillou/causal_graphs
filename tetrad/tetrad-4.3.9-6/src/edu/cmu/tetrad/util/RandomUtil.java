...
/**
 * Provides a common random number generator to be used throughout Tetrad, to avoid
 * problems that happen when random number generators are created more often than
 * once per millisecond. When this happens, the generators are synced, and there
 * is less randomness than expected.
 * <p/>
 * A seed can be set for the generator using the <code>setSeed</code> method. This
 * is useful if an experiment needs to be repeated under different conditions. The
 * seed for an experiment can be printed using the <code>getSeed</code> method.
 * <p/>
 * The 64-bit Mersenne Twister implementation from the COLT library is used to
 * generate random numbers.
 * <p/>
 * To see what distributions are currently supported, look at the methods of the class.
 * These many change over time.
 *
...
//        return uniform.nextIntFromTo(0, n - 1);
     ...
     * @param seed A long value. Once this seed is set, the behavior of the random
     * number generator is deterministic, so setting the seed can be used to
    ...
    /**
     * Returns the next random number drawn from a Poisson distribution with the
     * given mean.
     * @param lambda A positive real number equal to the expected number of occurrences
     * during a given interval. See Wikipedia.
     * @return Ibid.
     */
    ...
    /**
     * Returns Normal CDF value for a normal with the given mean and standard
     * deviation.
    ...
    /**
     * Returns the next random number drawn from the Beta distribution with the
     * given alpha and beta values. By changing the alpha and beta parameters,
     * radically different distibutions can be achieved. The Beta function is
    ...
    /**
     * Returns the next random number drawn from the von Mises distrbution with
     * the given degrees of freedom. Note that mu is assumed to be zero. See
     * Wikipedia.
    ...
    /**
     * Returns the next random number drawn from the Student T distribution with
    ...
    /**
     * Returns the next random number drawn from the Exponential distribution
    ...
    /**
     * Returns the next random number drawn from the Eponential Power distribution
     * with the given tau.
    ...
    /**
     * Returns the next random number drawn from teh given Chi Square distrubution
    ...
    /**
     * Returns the next random number drawn from the Gamma distribution with the
     * given alpha and lambda.
     * @param alpha The shape parameter.
     * @param lambda The scale parameter.
     * @return Ibid.
     */
    public double nextGamma(double alpha, double lambda) {
        return new Gamma(alpha, lambda, engine).nextDouble();
    }

    /**
     * Returns the next random number drawn from the Logarithmic distribution, with
     * the given p.
    ...
    /**
     * Returns the next random number drawn from the Breit Wigner distribution with
     * the given mean, gamma, and cut. I'm not sure of the identity of these parameters,
     * sorry.
    ...
    /**
     * Returns the next random number drawn from the given Hyperbolic distribution
     * with the given alpha and gamma.
     * @param alpha Real.
