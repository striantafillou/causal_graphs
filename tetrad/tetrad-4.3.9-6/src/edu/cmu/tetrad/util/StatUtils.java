...
//    }
     ...
     * @param p                    An array containing p-values to be tested in
     ...
     *                             <code>pValues </code> are correlated (true if
     *                             yes, false if no). If they are uncorrelated,
...
//        return -1.0 * submatrix.get(0, 1);
        ...
        // cov(X, Y | Z) = cov(X, Y) - cov(X, Z) inverse(var(Z)) cov(Z, Y)
        ...
        DoubleMatrix2D _zInverse = new Algebra().inverse(covZ);

        DoubleMatrix2D temp1 = new Algebra().mult(covXz, _zInverse);
        DoubleMatrix2D temp2 = new Algebra().mult(temp1, covZy);
